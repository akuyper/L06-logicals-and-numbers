...
The work for this case study is not aimed at producing something for a boss. It is aimed at practicing the with logicals and numerics. Therefore, providing and displaying code is expected. Raw tibble output is acceptable (meaning, you do not need to use `kable()` to make tables).
...
- Use `if_else()` to change the `public` variable to be `TRUE` if it is public and `FALSE` if it is not.
...
Provide appropriate visualizations or tables to help support the answer to each of the questions below. Code can be hidden, but visualizations and tables are expected to be nicely formatted.
